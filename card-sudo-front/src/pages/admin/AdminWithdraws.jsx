...
export default function AdminWithdraws() {
  ...
  const fetchWithdraws = async () => {
    try {
      ...
      setPagination(prev => ({
        ...prev,
        total_pages: Math.ceil((response.data.total || 0) / (response.data.per_page || 20)),
                  ...
                  <tr key={withdraw.id} className={`hover:bg-gray-700/30 ${withdraw.status === 'PENDING' ? 'bg-yellow-500/5' : ''}`}>
                    ...
                    <td className="px-4 py-4">
                      <div>
                        ...
                        <p className="text-xs text-gray-400 capitalize">{withdraw.pix_key_type}</p>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      {getStatusBadge(withdraw.status)}
                      {withdraw.rejection_reason && (
                        <p className="text-xs text-red-400 mt-1 max-w-[200px] truncate" title={withdraw.rejection_reason}>
                          {withdraw.rejection_reason}
                        </p>
                      )}
                    </td>
