...
export default function AdminDashboard() {
  ...
  // Dados para gráfico de transações por dia (volume)
  const salesChartData = {
    labels: stats?.charts?.transactions?.map(d => {
    ...
    datasets: [
      {
        label: 'Volume (R$)',
        data: stats?.charts?.transactions?.map(d => d.volume) || [],
        fill: true,
        borderColor: '#EAB308',
  ...
  const usersChartData = {
    labels: stats?.charts?.users?.map(d => {
      const date = new Date(d.date);
      return date.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' });
    }) || [],
    datasets: [
      {
        ...
        data: stats?.charts?.users?.map(d => d.count) || [],
  ...
  const statCards = [
    {
      ...
      link: '/admin/users',
      change: `+${stats?.users?.today || 0} hoje`
    },
    {
      name: 'Saques Pendentes',
      value: stats?.financial?.pending_withdraws_count || 0,
      icon: BanknotesIcon,
      color: 'from-yellow-500 to-yellow-600',
      link: '/admin/withdraws',
      change: `R$ ${parseFloat(stats?.financial?.pending_withdraws_amount || 0).toFixed(2)}`,
      urgent: (stats?.financial?.pending_withdraws_count || 0) > 0
    },
    {
      name: 'Anúncios Ativos',
      value: stats?.marketplace?.active_listings || 0,
      icon: ShoppingBagIcon,
      color: 'from-green-500 to-green-600',
      link: '/admin/listings',
      change: `${stats?.marketplace?.total_listings || 0} total`
    },
    {
      name: 'Volume de Vendas',
      value: `R$ ${parseFloat(stats?.marketplace?.total_sales_volume || 0).toFixed(2)}`,
      icon: ArrowsRightLeftIcon,
      color: 'from-purple-500 to-purple-600',
      link: '/admin/transactions',
      change: `${stats?.marketplace?.sold_listings || 0} vendas`
    }
  ];
