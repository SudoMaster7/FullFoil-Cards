...
export default function AdminReports() {
  ...
  const salesChartData = {
    labels: reportData?.charts?.transactions?.map(d => {
    ...
    datasets: [
      {
        label: 'Volume (R$)',
        data: reportData?.charts?.transactions?.map(d => d.volume) || [],
        fill: true,
        borderColor: '#EAB308',
  ...
  const usersChartData = {
    labels: reportData?.charts?.users?.map(d => {
      const date = new Date(d.date);
      return date.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' });
    }) || [],
    datasets: [
      {
        ...
        data: reportData?.charts?.users?.map(d => d.count) || [],
  ...
  const transactionTypesData = {
    labels: ['Vendas', 'Saques Aprovados', 'Saques Pendentes', 'Saques Rejeitados'],
    datasets: [
      {
        data: [
          reportData?.marketplace?.sold_listings || 0,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gray-800 rounded-xl p-5">
          ...
          <p className="text-sm text-green-400 mt-2">+{reportData?.users?.today || 0} hoje</p>
        </div>
        <div className="bg-gray-800 rounded-xl p-5">
          <p className="text-sm text-gray-400">Volume de Vendas</p>
          <p className="text-3xl font-bold text-white mt-1">
            R$ {parseFloat(reportData?.marketplace?.total_sales_volume || 0).toFixed(2)}
          </p>
          <p className="text-sm text-gray-500 mt-2">{reportData?.marketplace?.sold_listings || 0} vendas</p>
        </div>
        <div className="bg-gray-800 rounded-xl p-5">
          <p className="text-sm text-gray-400">Anúncios Ativos</p>
          <p className="text-3xl font-bold text-white mt-1">{reportData?.marketplace?.active_listings || 0}</p>
          <p className="text-sm text-gray-500 mt-2">{reportData?.marketplace?.total_listings || 0} total</p>
        </div>
        <div className="bg-gray-800 rounded-xl p-5">
          <p className="text-sm text-gray-400">Saques Pendentes</p>
          <p className="text-3xl font-bold text-yellow-500 mt-1">
            R$ {parseFloat(reportData?.financial?.pending_withdraws_amount || 0).toFixed(2)}
          </p>
          <p className="text-sm text-gray-500 mt-2">{reportData?.financial?.pending_withdraws_count || 0} solicitações</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-gray-800 rounded-xl p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-700/50 rounded-lg p-4">
              <p className="text-sm text-gray-400">Volume Total de Vendas</p>
              <p className="text-2xl font-bold text-green-400">
                R$ {parseFloat(reportData?.marketplace?.total_sales_volume || 0).toFixed(2)}
              </p>
            </div>
            <div className="bg-gray-700/50 rounded-lg p-4">
              <p className="text-sm text-gray-400">Novos Usuários (7 dias)</p>
              <p className="text-2xl font-bold text-blue-400">
                {reportData?.users?.last_7_days || 0}
              </p>
            </div>
            <div className="bg-gray-700/50 rounded-lg p-4">
